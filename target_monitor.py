...
def get_fulfillment(tcin, store_id, zip_code):
...
status = store_opts.get("order_pickup", {}).get("availability_status") or \
store_opts.get("in_store_only", {}).get("availability_status")
...
for tcin in TCINS:
for store in STORES:
status, qty = get_fulfillment(tcin, store["id"], store["zip"])
